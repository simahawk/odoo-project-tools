...
import os
changelog = sorted(changelog, key=lambda x: x["version"])
for ver in changelog:
    for typ, commits in ver["changes"].items():
        for i, commit in enumerate(commits):
            with open(f"changes.d/{i}.{typ}.rst", "w") as fd:
                fd.write(commit)
    os.system(f"towncrier build --yes --version {ver['version']} --date {ver['date']}")